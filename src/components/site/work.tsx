...
function ProjectRow({
    ...
    const inner = (
        <>
            ...
            <Mark p={p} />
            <span className="min-w-0 flex items-center gap-3 overflow-hidden">
                <span
                    className={`${titleClass} shrink-0 max-w-[55%] inline-flex items-center gap-2`}
                >
                    {p.kind === "product" ? (
                        <span
                            className="inline-flex items-center h-[16px] px-1.5 rounded-[3px] bg-bg-elevated border border-rule-subtle text-fg-muted font-mono text-[9px] font-semibold tracking-[0.1em] uppercase leading-none shrink-0"
                            aria-label="Live · paid product"
                        >
                            Live
                        </span>
                    ) : null}
...
export default function Work() {
    ...
    const preview = hover?.preview ?? null;

    return (
        <section className="py-10" id="work">
            <EyebrowLine label="02 · Work" count={`${projects.length}`} />
            <p className="text-sm leading-[1.65] text-fg-muted tracking-[-0.005em] m-0 mb-[18px]">
                Things I built end-to-end.{" "}
                <span className="text-fg-subtle">
                    <span className="inline-flex items-center h-[14px] px-1 rounded-[3px] bg-bg-elevated border border-rule-subtle text-fg-muted font-mono text-[8.5px] font-semibold tracking-[0.1em] uppercase align-middle mx-0.5">
                        Live
                    </span>{" "}
                    marks the ones people pay for.
                </span>
            </p>
            ...
            <WorkPreview src={preview ?? null} alt={hover ? `${hover.title} preview` : ""} />
        </section>
    );
}
